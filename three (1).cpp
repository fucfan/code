...
//
    ...
    //    nl;
            ...
            for(int i = 1 ; i <= n ; i++){
                for(int j = 1 ; j <= n ; j++){
                    sum = pref2[i][n] + pref1[n][j] - board[i][j];
                    // cout << i << ' ' << j << ' ' << sum << ' ' << sum + pref2[x][n] - board[x][j] << ' ' << sum + pref1[n][y] - board[i][y] << '\n';
                    if(i != x) new_ans = max(new_ans , sum + pref2[x][n] - board[x][j]);
                    if(j != y) new_ans = max(new_ans , sum + pref1[n][y] - board[i][y]);
                }
            }

            for(int i = 1 ; i < n ; i++){
                for(int j = i + 1 ; j <= n ; j++){
                    new_ans = max(new_ans , pref2[i][n] + pref2[j][n] + pref1[n][y] - board[i][y] - board[j][y]);
                    new_ans = max(new_ans , pref1[n][i] + pref1[n][j] + pref2[x][n] - board[x][i] - board[x][j]); 
                }
            }
