    ...
    void solve(){
        ...
        for(int t = 1; t <= n; t++){
            ...
            for(int mask = 0; mask < (1 << (n - 1)); mask++){
                ...
                cnt_rig += d[n].good;
                time_rig -= d[n].time_cook;

                sort(all(lef), cmp_lef);
                sort(rig.begin() + 1, rig.end(), cmp_rig);

                if(time_lef < 0 || (rig.size() && time_rig < 0 && -time_rig > rig[0].time_cook)) continue;

                int cur = 0;
                for(auto j : rig){
                    // cout << d[n].time_cook << ' ' << j.time_cook << ' ' << j.good << '\n';
                    cur += j.time_cook;
                    val += cur * j.good;
                }
                ...
                if(time_rig < 0){
                    ...
                    if(cnt_lef < cnt_rig){
                        val -= min(time_lef, rig[0].time_cook + time_rig) * cnt_rig;
                        val += min(time_lef, rig[0].time_cook + time_rig) * cnt_lef;
                    }
                }
                else{
                    if(cnt_lef < cnt_rig){
                        val -= min(time_lef, rig[0].time_cook) * cnt_rig;
                        val += min(time_lef, rig[0].time_cook) * cnt_lef;
                    }
                }
                ...
                //     cout << BIT(mask, i - 1);
                // }
                // cout << ' ' << time_lef << ' ' << time_rig << ' ' << cnt_lef << ' ' << cnt_rig << ' ' << val << ' ' << rig[1].time_cook << '\n';
